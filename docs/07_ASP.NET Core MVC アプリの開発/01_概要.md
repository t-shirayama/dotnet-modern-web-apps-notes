...
ASP.NET Core MVC アプリの開発では、HTTP request（リクエスト。ブラウザーやクライアントからサーバーに送られる要求）が middleware、routing、controller、model binding、validation、filter、view / response（レスポンス。サーバーから返す結果）を通って処理されます。
